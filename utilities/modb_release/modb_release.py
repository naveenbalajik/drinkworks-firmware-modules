...
#
# Script to generate an ESP32 firmware release packages for Drinkworks Model-B Appliance.
# A separate release package is generated for PIC18 and PIC32 systems
#
...
def create_readme( logLines, ReleasePath, version, build, timestamp, internal, ZipPath ):
    ...
    with open(readmeFile, "w") as readme:
        ...
        readme.write( f'Version: {version}\n' )
        readme.write( f'Build:   {build}\n' )
...
#
#	Update the common log file
#
def update_log_file( logFile, versionBuild, timestamp, p18File, p32File, messages):
        ...
        if not logLines[n].startswith( "#"):
            if logLines[n].isspace():
                ...
                if (versionBuild in logLines[n]) and not ("(deleted)" in logLines[n]):
                    print( f'Log already has note for version: {versionBuild} - tag as deleted' )
    ...
    newNote = f'{versionBuild} {timestamp}'
    if args.internal:
        newNote += " (internal)\n"
    else:
        newNote += "\n"
    ...
    logLines.append( f'  P18 firmware: {p18File}\n' )
    logLines.append( f'  P32 firmware: {p32File}\n' )
	
    # if a message was specified, it can be multi-line, append each line
    if messages:
        for message in messages:
            logLines.append( f'  {message}\n' )
...
if __name__ == "__main__":
    ...
    # Construct programming arguments file
    ...
    logFile = f'{releasePath}\\release.log'
    logLines = update_log_file( logFile, versionBuild, dateTimeFormat, p18File, p32File, args.message )
